...
const getData = (el: HTMLElement | null): string | null =>
  (el?.__vnode?.props?.[KEY_PROPS_DATA] as string | undefined) ??
  getComponentData(el) ??
...
const toggleCombo = ref(
  inspectorOptions.toggleComboKey?.toLowerCase?.()?.split?.("-") ?? false
);
const disableInspectorOnEditorOpen = ref(
...
const containerVisible = computed(() => {
  ...
  return (
    ...
    (toggleButtonVisibility === "active" && enabled.value)
  );
});

const containerPosition = computed(() =>
  inspectorOptions.toggleButtonPos
    .split("-")
    .map((p) => `${p}: 15px;`)
    .join("")
);
...
const onKeydown = (event: KeyboardEvent) => {
  ...
  const isCombo = toggleCombo.value
    ? toggleCombo.value.every((key) => isKeyActive(key, event))
    : false;
...
const getTargetNode = (e: MouseEvent): TargetNodeResult => {
  ...
  if (!path.length) {
    return { targetNode: null, params: null };
  }

  const ignoreIndex = path.findIndex((node) =>
    (node as HTMLElement)?.hasAttribute?.(KEY_IGNORE)
  );
  const targetNode = path
    .slice(ignoreIndex + 1)
    .find((node) => getData(node as HTMLElement)) as HTMLElement | undefined;
  ...
  return {
    targetNode,
    params: match
      ? { file, line: Number(line), column: Number(column), title: file }
      : null,
  };
};
...
const closeOverlay = () => {
  overlayVisible.value = false;
  linkParams.value = { file: "", line: 0, column: 0 };
};
